...
/* Implemented in board file (e.g. bl_can_hw_f103.c) */
...
void bl_can_hw_heartbeat(void) __attribute__((weak));
...
static void send_rsp(uint8_t cmd, uint8_t status, const uint8_t *extra, uint8_t extra_len)
{
    ...
    if (extra != 0 && extra_len > 0u) {
        if (extra_len > 6u) {
            extra_len = 6u; /* bytes[2..7] */
        }
        memcpy(&frame[2], extra, extra_len);
    }
    /* Always transmit full classic CAN frame (DLC=8), pad unused with 0.
     * Avoids tools showing "truncated" short frames and keeps layout stable. */
...
void bl_can_poll(void)
{
    ...
    /* Pause alive frame during an update session (after ERASE until ABORT/reset) */
    if (s_image_size == 0u) {
        bl_can_hw_heartbeat();
    }
}
